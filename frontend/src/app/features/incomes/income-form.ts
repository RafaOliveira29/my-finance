  ...
  template: `
    <section class="page">
      ...
      @if (loading()) {
        ...
        <p class="state">
          Cadastre antes uma categoria do tipo <strong>Receita</strong> —
        ...
        <form class="form-card" [formGroup]="form" (ngSubmit)="submit()">
          ...
          <div class="field-row">
            ...
            <label class="field">
              Dia da competência
              <input type="number" formControlName="competenceDay" min="1" max="31" />
              @if (invalid('competenceDay')) {
                <small class="field-error">Informe um dia entre 1 e 31.</small>
              }
            </label>
          </div>
          ...
          <div class="form-actions">
            ...
            <a class="btn-ghost" routerLink="/income-sources">Cancelar</a>
          </div>
        </form>
      }
    </section>
...
export class IncomeForm {
  ...
  /** Receita só aceita categoria de receita — evita erro de validação no backend. */
  readonly categories = computed(() => this.allCategories().filter((c) => c.type === 'Income'));

  readonly form = this.fb.nonNullable.group({
    description: ['', [Validators.required, Validators.maxLength(150)]],
    categoryId: ['', [Validators.required]],
    defaultAmount: [0, [Validators.required, Validators.min(0)]],
    competenceDay: [5, [Validators.required, Validators.min(1), Validators.max(31)]],
  ...
  invalid(control: 'description' | 'defaultAmount' | 'competenceDay' | 'startDate'): boolean {
  ...
  submit(): void {
    ...
    const payload = {
      ...
      competenceDay: value.competenceDay,
    ...
    request.subscribe({
      next: () => this.router.navigateByUrl('/income-sources'),
      error: (err) => {
        this.error.set(apiErrorMessage(err, 'Não foi possível salvar a receita.'));
  ...
  private loadSource(id: string): void {
    this.service.getById(id).subscribe({
      ...
      error: (err) => {
        this.error.set(apiErrorMessage(err, 'Receita não encontrada.'));
        this.loading.set(false);
      },
    });
  }

...
- ▶️ **Próxima: Fase 2 (Cadastros base — Categorias, Receitas, Despesas fixas)**.

> **➡️ PONTO DE RETOMADA:** Fases 0 e 1 concluídas e verificadas. **Próximo passo: Fase 2 — Cadastros base** (Categorias com cor/ícone, Fontes de receita, Despesas fixas; CRUD base + `HasQueryFilter` por `UserId` nas entidades; prova CA008–CA019, CA062/63/65/76). **Card vivo espelhado no cérebro:** `Cerebro/projetos/MyFinance/DOC.md` (regra [[documentacao-viva]]); este `DOC.md` do repo é a doc viva completa. **Projeto vive no repo git `my-finance`** (GitHub: `RafaOliveira29/my-finance`); ainda **não commitado/pushado** por mim (aguardando seu pedido).
...
>
> **Onde paramos:** Fases 0 e 1 concluídas e verificadas → **próxima: Fase 2**.
...
- ⬜ **Fase 2 — Cadastros base** (E2/E3/E11 core) — **PRÓXIMA**. Passos: 1) `HasQueryFilter` por `UserId` (escopo de tenant no EF); 2) `Category` — CRUD com cor/ícone, RESTRICT se em uso; 3) `IncomeSource` — CRUD (valor padrão, dia de competência, recorrência); 4) `ExpenseSource` fixa — CRUD (dia de vencimento); 5) CRUD base parametrizado + telas no front; 6) migration; 7) testes + Mapa de teste. **Prova:** CA008–CA019, CA062–CA063, CA065, CA076.
- ⬜ **Fase 3 — Competência & pagamentos** (E7/E8/E4) — *coração do dinheiro*. Passos: 1) `MonthlyEntry` + `Nature` + CHECK de exatamente-uma-fonte + UNIQUE parciais; 2) `IMonthlyEntryGenerator` (geração idempotente do mês — RN04/RN10/RN16); 3) lançamento avulso (`SourceType=Manual`); 4) `Payment` + `IPaymentService` (parcial/total, guarda de superpagamento, estorno, status derivado, transação + RowVersion); 5) telas no front; 6) migration; 7) testes de dinheiro (RNF08) + Mapa de teste. **Prova:** CA020–CA023, CA038–CA049, CA079.
